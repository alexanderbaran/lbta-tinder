...
class SwipingPhotosController: UIPageViewController, UIPageViewControllerDataSource, UIPageViewControllerDelegate {
    ...
    var cardViewModel: CardViewModel! {
        didSet {
            controllers = cardViewModel.imageUrls.map({ (imageUrlString: String) -> UIViewController in
                let photoController = PhotoController(imageUrlString: imageUrlString)
    ...
    private func setupBarViews() {
        ...
        /* Skip using safeAreaLayoutGuide here because of blinking bug. */
        var paddingTop: CGFloat = 8
        if !isCardViewMode {
            paddingTop = UIApplication.shared.statusBarFrame.height + paddingTop
        }
...
//        setViewControllers([controllers.first!], direction: .forward, animated: false)
    ...
    private func disableSwipingAbility() {
        view.subviews.forEach { (v: UIView) in
...
class PhotoController: UIViewController {
    ...
    let imageView: UIImageView = {
        let iv = UIImageView(image: UIImage(named: "kelly1"))
        iv.contentMode = .scaleAspectFill
        iv.clipsToBounds = true
        return iv
    }()
    
    // Provide an initializer that takes in a URL instead of image.
